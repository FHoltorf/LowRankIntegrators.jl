struct KSLStepper <: LowRankStepper end
struct LSKStepper <: LowRankStepper end
struct KSLLSKStepper <: LowRankStepper end

@concrete struct KSLStepperCache <: LowRankStepperCache
    K0
    L0
    dK
    dL
...
state(cache::KSLStepperCache) = cache.X

initialize_cache(prob::MatrixDEProblem, ::LSKStepper, SA) = initialize_cache(prob, KSLStepper(), SA)
initialize_cache(prob::MatrixDEProblem, ::KSLLSKStepper, SA) = initialize_cache(prob, KSLStepper(), SA)

function initialize_cache(prob, R::KSLStepper, SA)
    ...
    KSLStepperCache(K0, L0, dK, dL, dS, X, sparse_approximator_cache)
end
...
function initialize_sparse_approximation_cache(SA::SparseApproximation, X0, ::KSLStepper)
...
function retracted_step!(cache, model::AbstractLowRankModel, t, h, ::KSLStepper, SA)
    K_step!(cache, model, t, h, SA)
    S_step!(cache, model, t, h, SA)
    L_step!(cache, model, t, h, SA)
end

function retracted_step!(cache, model::AbstractLowRankModel, t, h, ::LSKStepper, SA)
    L_step!(cache, model, t, h, SA)
    S_step!(cache, model, t, h, SA)
    K_step!(cache, model, t, h, SA)
end

function retracted_step!(cache, model::FactoredLowRankModel, t, h, ::KSLLSKStepper, SA)
    retracted_step!(cache, model, t, h/2, KSLStepper(), SA)
    retracted_step!(cache, model, t, h/2, LSKStepper(), SA)
end

function retracted_step!(cache, model::SparseLowRankModel, t, h, ::KSLLSKStepper, SA)
    retracted_step!(cache, model, t, h/2, KSLStepper(), SA)
    update_cache!(cache, SA)
    retracted_step!(cache, model, t + h/2, h/2, LSKStepper(), SA)
end

# for outofplace factored model 
function K_step!(cache::KSLStepperCache, model::FactoredLowRankModel{false}, t, h, ::Missing)
    @unpack X, K0 = cache
...
function S_step!(cache::KSLStepperCache, model::FactoredLowRankModel{false}, t, h, ::Missing)
    @unpack X = cache
    dS = Matrix(X.U'*F(model, X, t)*X.V)
    X.S .-= h*dS
end
function L_step!(cache::KSLStepperCache, model::FactoredLowRankModel{false}, t, h, ::Missing)
    @unpack X, L0 = cache
    mul!(L0, X.V, X.S')
...
function K_step!(cache::KSLStepperCache, model::SparseLowRankModel{true}, t, h, SA::SparseApproximation)
...
function S_step!(cache::KSLStepperCache, model::SparseLowRankModel{true}, t, h, SA::SparseApproximation)
...
function L_step!(cache::KSLStepperCache, model::SparseLowRankModel{true}, t, h, SA::SparseApproximation)
...
function update_cache!(cache::KSLStepperCache, SA::SparseApproximation)
