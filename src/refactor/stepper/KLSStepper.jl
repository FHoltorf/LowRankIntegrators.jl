struct KLSStepper <: LowRankStepper end 

@concrete struct KLSStepperCache <: LowRankStepperCache
    K0
    L0
    U1
    S1
...
state(cache::KLSStepperCache) = cache.X

function initialize_cache(prob, R::KLSStepper, SA)
    ...
    KLSStepperCache(K0, L0, U1, S1, V1, dK, dL, dS, M, N, X, sparse_approximator_cache)
end
...
function initialize_sparse_approximation_cache(SA::SparseApproximation, X0, ::KLSStepper)
...
function retracted_step!(cache, model::AbstractLowRankModel, t, h, ::KLSStepper, SA)
...
function K_step!(cache::KLSStepperCache, model::FactoredLowRankModel{false}, t, h, ::Missing)
...
function L_step!(cache::KLSStepperCache, model::FactoredLowRankModel{false}, t, h, ::Missing)
...
function S_step!(cache::KLSStepperCache, model::FactoredLowRankModel{false}, t, h, ::Missing)
...
function K_step!(cache::KLSStepperCache, model::SparseLowRankModel{true}, t, h, SA::SparseApproximation)
...
function L_step!(cache::KLSStepperCache, model::SparseLowRankModel{true}, t, h, SA::SparseApproximation)
...
function S_step!(cache::KLSStepperCache, model::SparseLowRankModel{true}, t, h, SA::SparseApproximation)
...
function update_cache!(cache::KLSStepperCache, SA::SparseApproximation)
