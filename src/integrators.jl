struct LieTrotterProjectorSplitting{sType, lType, kType}
...
struct PrimalLieTrotterProjectorSplitting{sType, lType, kType} <: AbstractDLRAlgorithm
    alg_params::LieTrotterProjectorSplitting{sType, lType, kType}
    function PrimalLieTrotterProjectorSplitting(S_rhs = nothing, L_rhs = nothing, K_rhs = nothing; 
                                                S_alg=Tsit5(), L_alg = Tsit5(), K_alg = Tsit5())
        return new{typeof(S_alg), typeof(L_alg), typeof(K_alg)}(LieTrotterProjectorSplitting(S_rhs,L_rhs,K_rhs,S_alg,L_alg,K_alg))          
    end
end
