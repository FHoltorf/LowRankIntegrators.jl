...
sol_lra = Observable(Matrix(integrator.sol.Y[1]))
sol_full = Observable(sol.u[1])
error = Observable([norm(sol.u[1] - Matrix(integrator.sol.Y[1]))])
ts = Observable([0.0])
...
for (i,t) in enumerate(integrator.sol.t)
    sol_lra[] = Matrix(integrator.sol.Y[i])
    sol_full[] = sol(t)
    push!(error.val, norm(Matrix(integrator.sol.Y[i]) - sol(t)))
    push!(ts.val, t)
    ts[] = ts[]
    error[] = error[]
    sleep(0.03) 
