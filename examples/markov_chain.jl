...
Srows = [sparse(shift(ν[r][1],N)) for r in 1:Nrxn] # shift operators
Scols = [sparse(shift(ν[r][2],N)') for r in 1:Nrxn] # shift operators
...
P_lr = truncated_svd(P_init, tol = 1e-6)
@show rank(P_lr)
lr_prob = MatrixDEProblem((x,t) -> cme(x, (A, Asum, Srows, Scols), t), P_lr, (0.0, 10.0))
