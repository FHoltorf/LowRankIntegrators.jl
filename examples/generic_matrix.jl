using LowRankIntegrators, Plots, LinearAlgebra
N = 100
Ws = []
for k in 1:2
    W = randn(N,N)
...
S_kwargs, K_kwargs, L_kwargs = Dict(), Dict(), Dict()
alg = Euler()
solvers = [ProjectorSplitting(PrimalLieTrotter()), ProjectorSplitting(DualLieTrotter()), ProjectorSplitting(Strang()), UnconventionalAlgorithm(), RankAdaptiveUnconventionalAlgorithm(tol, rmax = 20)]
error = Dict()
for r in r_range
...
solver_names = Dict(solvers[1] => "Primal Lie Trotter Projector Splitting", 
                    solvers[2] => "Dual Lie Trotter Projector Splitting",
                    solvers[3] => "Strang Projector Splitting",
                    solvers[4] => "Unconventional Algorithm",
                    solvers[5] => "Rank Adaptive Unconventional Algorithm")
error_comparison = plot(yscale = :log, xscale = :log, xlabel = "step size", ylabel = "error", legend = :bottomleft)

colors = Dict(solvers[1] => :blue, solvers[2] => :red, solvers[3] => :green, solvers[4] => :dodgerblue, solvers[5] => :orange)
