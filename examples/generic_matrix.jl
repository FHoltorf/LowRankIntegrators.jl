using LowRankIntegrators, Plots, LaTeXStrings
N = 100
Ws = []
for k in 1:2
    W = randn(N,N)
...
solvers = [PrimalLieTrotterProjectorSplitting(), StrangProjectorSplitting(), UnconventionalAlgorithm()]
error = Dict()
for r in r_range
    ...
    for dt in dt_range
        for solver in solvers
            integrator = solve(prob, solver, dt)
